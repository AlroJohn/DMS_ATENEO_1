...
export class DocumentService {
  ...
  async getOwnedDocuments(userId: string, page: number = 1, limit: number = 10) {
    try {
      ...
      // Get all document additional details created by this user's account
      const documentDetails = await prisma.documentAdditionalDetails.findMany({
        where: {
          account_id: user.account_id  // Get documents created by this user's account
        },
        select: {
          document_id: true
        }
      ...
      console.log('📍 [getOwnedDocuments] Total document details found for user:', documentDetails.length);

      const relevantDocumentIds = documentDetails.map((detail: any) => detail.document_id);
      ...
      const [documents, total] = await Promise.all([
        prisma.document.findMany({
          where: {
            ...
            status: {
              notIn: ['deleted']  // Show documents with any status except deleted
            }
          },
          skip,
          take: limit
        ...
        prisma.document.count({
          where: {
            ...
            status: {
              notIn: ['deleted']  // Count documents with any status except deleted
            }
          }
        })
      ]);
  ...
  async createDocumentWithFile(documentData: any, file: Express.Multer.File | undefined, userId: string) {
    ...
    if (file) {
      ...
      try {
        ...
        const metadataRecord = await this.prismaAny.documentMetadata.create({
          data: {
            file_id: createdFile.file_id, // Associate metadata with the specific file
            ...metadata,
            file_size: BigInt(metadata.file_size || 0),
          }
        });
        
        // Log the inserted metadata as JSON
        console.log('Document metadata inserted:', JSON.stringify(metadataRecord, null, 2));
      } catch (error) {
        console.error('Failed to extract or save document metadata:', error);
      }
    }
  ...
  async uploadFilesToDocument(documentId: string, files: Express.Multer.File[], userId: string) {
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        ...
        const metadataRecord = await this.prismaAny.documentMetadata.create({
          data: {
            file_id: created.file_id, // Associate metadata with the specific file
            ...metadata,
            file_size: BigInt(metadata.file_size || 0),
          }
        });
